...
namespace ImGui
{
    ...
    IMGUI_API bool          RadioButton(const char* label, bool actives);                    // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
...
struct ImGuiIO
{
    ...
    IMGUI_API void  ClearInputCharacters();                     // AllClear the text input buffer manually
...
struct ImFontAtlas
{
    ...
    IMGUI_API void              ClearInputData();           // AllClear input data (all ImFontConfig structures including sizes, TTF data, glyph ranges, etc.) = all the data used to build the texture and fonts.
    IMGUI_API void              ClearTexData();             // AllClear output texture data (CPU side). Saves RAM once the texture has been copied to graphics memory.
    IMGUI_API void              ClearFonts();               // AllClear output font data (glyphs storage, UV coordinates).
    IMGUI_API void              Clear();                    // AllClear all input and output.
